...
<body>
    ...
    <div class="main-title uppercase">COTIZACIÓN / PROFORMA</div>
    ...
    <div style="position: relative;">
        ...
        <table class="header-table">
            <tr>
                ...
                <td class="meta-container">
                    <table style="width: 120pt; border-collapse: collapse; margin-left: auto;">
                        <tr>
                            <td class="meta-label uppercase" style="text-align: left; padding-bottom: 2pt;">N° PROFORMA:</td>
                            <td class="meta-value text-right" style="padding-bottom: 2pt;">{{ $quotation->formatted_number }}</td>
                        </tr>
                        <tr>
                            <td class="meta-label uppercase" style="text-align: left; padding-bottom: 2pt;">FECHA:</td>
                            <td class="meta-value text-right" style="padding-bottom: 2pt;">{{ $quotation->date->format('d/m/Y') }}</td>
                        </tr>
                        <tr>
                            <td class="meta-label uppercase" style="text-align: left;">VALIDEZ:</td>
                            <td class="meta-value text-right">{{ $quotation->valid_until->format('d/m/Y') }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
        ...
        <table class="client-bar">
            <tr>
                ...
                <td class="client-value uppercase">{{ $quotation->client->name }}</td>
                <td class="client-label uppercase text-right" style="width: 10%;">DOC:</td>
                <td class="client-value text-right" style="width: 20%;">{{ $quotation->client->document_number ?? 'S/N' }}</td>
            </tr>
        </table>
        ...
        <div class="summary-container">
            <div style="float: left; width: 55%;" class="amount-in-words">
                @php
                    $integerPart = floor($quotation->total_payment);
                    $decimalPart = round(($quotation->total_payment - $integerPart) * 100);
            ...
            <table class="totals-table">
                <tr>
                    ...
                    <td class="text-right font-bold" style="width: 70pt;">{{ number_format((float)$quotation->subtotal, 2) }}</td>
                </tr>
                @if($quotation->global_discount> 0)
                <tr>
                    <td class="text-right font-bold" style="color: #b91c1c; font-size: 8pt;">Descuento Adic. Bs</td>
                    <td class="text-right font-bold" style="color: #b91c1c;">-{{ number_format((float)$quotation->global_discount, 2) }}</td>
                </tr>
                @endif
                <tr class="total-pay-row">
                    <td class="text-right total-pay-label uppercase">TOTAL Bs</td>
                    <td class="text-right total-pay-value">{{ number_format((float)$quotation->total_payment, 2) }}</td>
                </tr>
            </table>
            <div class="clear"></div>
        </div>
    ...
    <div class="footer-thanks uppercase">
        Esta proforma tiene una validez de 15 días. Precios sujetos a cambio.
    </div>
    <div class="footer-system">
        Generado por Sistema Inventory & POS | {{ date('d/m/Y H:i:s') }}
    </div>
